...
export function QuestionBankView({ isAdmin }: { isAdmin: boolean }) {
  ...
  const fetchAdminQuestions = async (isLoadMore = false) => {
    ...
    try {
      ...
      let supaList: BankQuestion[] = [];
      let usedSupabase = false;

      try {
        const offset = isLoadMore ? questions.length : 0;
        let query = supabase.from('question_bank').select('*').order('created_at', { ascending: false }).range(offset, offset + 29);
        
        const { data, error } = await query;
        if (!error && data && data.length > 0) {
          supaList = data.map((row: any) => ({
            id: row.id,
            text: row.text,
            options: row.options || [],
        ...
        console.warn("Supabase fetch question_bank error:", supaErr);
      }

      if (usedSupabase) {
        let flaggedList: BankQuestion[] = [];
        if (!isLoadMore) {
          try {
            const { data: flagged } = await supabase.from('question_bank').select('*').eq('has_image_warning', true).limit(20);
            if (flagged) {
               flaggedList = flagged.map((row: any) => ({
                id: row.id,
                text: row.text,
                options: row.options || [],
                correctAnswer: row.correct_answer || '',
        ...
        const all = isLoadMore ? [...questions, ...supaList] : [...flaggedList, ...supaList];
